...
def _is_valid_user(user: dict) -> bool:
    return isinstance(user, dict) and "name" in user and "congratulation_date" in user
...
def test_each_date_is_in_valid_format(users):
    result = get_upcoming_birthdays(users)
    assert all(
        re.match(r"\d{4}\.\d{2}\.\d{2}", user["congratulation_date"]) for user in result
    )
...
def _is_congratulation_date_not_on_weekend(user: dict) -> bool:
    return datetime.strptime(user["congratulation_date"], "%Y.%m.%d").weekday() not in [
        5,
        6,
    ]
